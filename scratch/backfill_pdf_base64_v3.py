...
async def backfill_v3():
    print("Iniciando backfill de PDF base64 (v3 com melhor seleção de mailbox)...")
            ...
            for f in faturas:
                try:
                    ...
                    if f.pdf_path:
                        ...
                        if path.exists():
                            f.pdf_base64 = base64.b64encode(path.read_bytes()).decode('utf-8')
                            count += 1
                            continue
                    ...
                    if f.gmail_message_id:
                        ...
                        # Lista de pastas para tentar
                        mailboxes = ["INBOX", '"[Gmail]/Todos os e-mails"', '"[Gmail]/All Mail"']
                        for mb in mailboxes:
                            try:
                                status, _ = mail.select(mb, readonly=True)
                                ...
                                if status == "OK" and search_data[0]:
                                    ...
                                    if res == "OK":
                                        ...
                                        if atts:
                                            ...
                                            count += 1
                                            found = True
                                            print(f"  [OK] Recuperado de {mb}.")
                                            break
                            except Exception as e:
                                logger.debug(f"Erro ao buscar na pasta {mb}: {e}")
                                continue
                        
                        if not found:
                            logger.warning(f"  [AVISO] Mensagem não encontrada no Gmail: {f.gmail_message_id}")
